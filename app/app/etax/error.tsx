...
export default function Error({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 text-center bg-white">
      ...
      <p className="text-sm text-[#6B6459] mb-6">{error.message}</p>
      <button onClick={reset} className="px-6 py-3 bg-[#1A1A1A] text-white rounded-xl text-sm font-medium">再試行</button>

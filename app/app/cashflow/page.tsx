...
export default function CashflowPage() {
  ...
  useEffect(() => {
    ...
    const doneKey = `ninjaAutoAdded_${ym}`
    if (localStorage.getItem(doneKey)) return  // 今月はもう処理済み

    const recurring = cashflowPlan.filter(p => p.repeat !== 'once' && p.ym < ym)
    if (recurring.length === 0) return

    // 今月分がまだ登録されていないものを自動追加
    const thisMonthIds = cashflowPlan.filter(p => p.ym === ym).map(p => p.name)
    let added = 0
    recurring.forEach(p => {
      if (!thisMonthIds.includes(p.name)) {
        addCashflowItem({
  ...
  function autoAddRecurring() {}  // marker
  ...
  function printMonthlySummary() {
    ...
    win.document.write(`<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8">
      <title>月次収支サマリー ${label}</title>
      <style>
        body{font-family:'Noto Sans JP',sans-serif;padding:32px;color:#1A1A1A;max-width:640px;margin:0 auto;}
        h1{font-size:20px;border-bottom:2px solid #2F5D50;padding-bottom:8px;color:#2F5D50;}
        table{width:100%;border-collapse:collapse;margin-top:12px;}
        th{background:#2F5D50;color:#fff;padding:8px;font-size:12px;text-align:right;}
        th:first-child{text-align:left;}
        td{padding:7px 8px;border-bottom:1px solid #E5E0D8;font-size:13px;text-align:right;}
        td:first-child{text-align:left;}
        .pos{color:#1E5C3A;font-weight:600;}.neg{color:#8B2020;font-weight:600;}
        .note{font-size:11px;color:#9A9288;margin-top:24px;}
      </style></head><body>
      <h1>月次収支サマリー — ${label}</h1>
      <p style="font-size:12px;color:#6B6459;">作成日: ${new Date().toLocaleDateString('ja-JP')} / 忍者帳場</p>
      <table>
        <tr><th>項目</th><th>金額</th></tr>
        ${months.filter(m => m.ym === ym || m.ym < ym).slice(-3).map(m =>
          `<tr><td>${m.label}${m.isForecast?'（予）':''}</td>
           <td class="${m.net>=0?'pos':'neg'}">${m.net>=0?'+':''}¥${m.net.toLocaleString()}</td></tr>`
        ).join('')}
      </table>
      <p class="note">※ 本資料はAIによる概算です。実際の数値は帳簿を確認ください。</p>
      </body></html>`)
    win.document.close()
    win.print()
  }
  return (
    <div className="min-h-dvh pb-20 bg-white">
      ...
      <div className="max-w-lg mx-auto px-4 pt-5">
        ...
        {/* 月次サマリーPDF出力 */}
        <button onClick={printMonthlySummary}
          className="w-full py-3 border border-[#E5E0D8] rounded-xl text-sm text-[#6B6459] mb-2 bg-[#F8F8F8]">
          📄 今月の収支サマリーをPDF出力
        </button>

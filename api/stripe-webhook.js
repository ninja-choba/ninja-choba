...
// 統合前の「ひとり版（solo）」は、統合後の「事業版（pro）」と同じものとして扱う。
// クライアント側（isPaidPlan()等）はsolo/pro両方を有料プランとして認識するため、
// solo のまま保存しても機能自体は壊れないが、奥の間の表示等をproに揃えるため
// ここで正規化しておく（2026-08-26 ひとり版・事業版の統合）
function normalizePlanName(rawPlan) {
  if (!rawPlan) return rawPlan;
  var noYearly = rawPlan.replace(/_yearly$/, '');
  return noYearly === 'solo' ? 'pro' : noYearly;
}
...
export default async function handler(req) {
  ...
  // STRIPE_PRICE_SOLO（旧ひとり版）とSTRIPE_PRICE_PRO（旧事業版・980円）は、
  // どちらも統合後の「pro（事業版）」に対応させる。旧事業版（980円）で契約中の方が
  // まだ現行のPriceへ移行していない期間も、正しく「事業版」として扱われるようにするため
  //
  // 【2026-09-22 不動産版を廃止】
  // アプリ本体から不動産管理機能を削除し、契約者がいないことも確認済みのため、
  // STRIPE_PRICE_REALESTATEのマッピングを削除した。万一この環境変数が残って
  // いても、対応するPrice IDが来ることはもう無いため実害はない
  const PRICE_TO_PLAN = {
    [process.env.STRIPE_PRICE_SOLO]:        'pro',
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // 年払い（pro_yearly等）は基本プラン名（pro）に正規化し、
        // 統合前の'solo'も'pro'に正規化して保存する。
        // クライアント側のisPaidPlan()/getPlan()はsolo/proのみを認識するため、
      ...
      case 'customer.subscription.updated': {
        ...
        // '_yearly'・'solo'を正規化して基本プラン名に揃える。
        let plan = PRICE_TO_PLAN[priceId];
        if (!plan) {
          const metaPlan = sub.metadata?.plan;
          plan = metaPlan ? normalizePlanName(metaPlan) : 'free';
        }

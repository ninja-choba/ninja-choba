...
// 見出し（文字ロゴ）部分。画像を使わず、どのメールソフトでも確実に表示される
// 装飾のみで構成している。
//   ・深緑の帯（忍者帳場のブランドカラー）に、白抜きの大きな文字ロゴ
//   ・「帳」の一文字を朱色の角印ふうに添えて、和の意匠に
// display:flex等、メールクライアントで崩れやすい指定は使わず、
// line-heightで縦位置を合わせる古典的な手法にしている
const HEADER_HTML = `<div style="text-align:center;margin:0 0 32px;padding:40px 24px 30px;background:#1e4030;border-radius:2px 2px 0 0">
    <p style="font-size:34px;font-weight:bold;letter-spacing:12px;color:#fff;margin:0;text-indent:12px">忍者帳場</p>
    <p style="font-size:11px;color:rgba(255,255,255,0.6);letter-spacing:4px;margin:10px 0 18px">NINJA CHOBA</p>
    <span style="display:inline-block;width:30px;height:30px;line-height:30px;background:#a63b2e;color:#fff;font-size:14px;font-weight:bold;border-radius:3px">帳</span>
  </div>`;

const CONFIRM_HTML = `<div style="font-family:'Hiragino Mincho ProN','Yu Mincho',Georgia,serif;max-width:560px;margin:0 auto;padding:0 24px 40px;background:#fff;color:#1a1a1a">
  ${HEADER_HTML}
...
const RECOVERY_HTML = `<div style="font-family:'Hiragino Mincho ProN','Yu Mincho',Georgia,serif;max-width:560px;margin:0 auto;padding:0 24px 40px;background:#fff;color:#1a1a1a">
  ${HEADER_HTML}
  <p style="font-size:16px;line-height:2;margin-bottom:8px">拝啓</p>
  <p style="font-size:15px;line-height:2;margin-bottom:24px">
...
const MAGIC_LINK_HTML = `<div style="font-family:'Hiragino Mincho ProN','Yu Mincho',Georgia,serif;max-width:560px;margin:0 auto;padding:0 24px 40px;background:#fff;color:#1a1a1a">
  ${HEADER_HTML}
  <p style="font-size:16px;line-height:2;margin-bottom:8px">拝啓</p>
  <p style="font-size:15px;line-height:2;margin-bottom:24px">
